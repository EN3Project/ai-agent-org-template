...
phase_2:
  ...
  done_when: "verdict が PASS / CONDITIONAL_PASS / FAIL のいずれかで確定し、devil's advocate パスが完了"
...
本手順書はユーザーが自力で組織を構築する場合のガイドである。AI エージェントに組織設計を支援させる場合は、factory-side design reference である `OrgDesign.md` を使用する。

- **OrgDesign ワークフローの Phase 0（協調的要件定義）** が本手順書の Step 1 に対応する
- **OrgDesign ワークフローの Phase 1（組織設計）** が本手順書の Step 2 + Step 3 に対応する
- **OrgDesign エージェント定義（`org_designer.md`）** は本手順書のプリセット定義と同一の5プリセット（Minimal / Research / Development / Content / Full）を使用する
